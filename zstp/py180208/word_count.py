...
def readtext(path=path):
    ...
    texts=open(path,'r',encoding='gbk').read()
    return texts.strip('\n').strip()
...
def dict_df(df_word):
    ...
    for i in range(len(df_word)):
        ...
        if x['ind'] not in df_dict:
            df_dict[x['ind']]={}
            df_dict[x['ind']]={'name':x['out'] ,'itemStyle': {'normal':{'colour':x['color']}},'value':1000-x['ind']*3}
...
def cloud_word(word_freq):
    ...
    :return: 词云图
    """
    word_cloud=WordCloud(font_path = "C:\\anacon\\Lib\\site-packages\\msyh.ttf",
